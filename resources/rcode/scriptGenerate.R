...
# Filtra os sensores encontrados, deixando apenas as medias dos sensores
...
sensoresRemovidos <- dados %>% dplyr::select_if(list(~all(is.na(.)))) %>% names()
dados <- dados %>% dplyr::select_if(list(~any(!is.na(.))))
...
anemometros <- dados %>% dplyr::select(grep("AN-", names(dados)),grep("AN_", names(dados)))
...
if(nrow(sensores) == 0) {
  message("A torre nao possui registros de sensores na data especificada.\n")
}
...
if(length(sensoresRemovidos) > 0) {
  message("-\n")
  cat(paste0(length(sensoresRemovidos)), " sensores foram removidos pois nao continham dados:\n")
  cat(sensoresRemovidos)
}
