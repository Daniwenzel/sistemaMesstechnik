...
if(nrow(primeiraSensores) != 0 & nrow(segundaSensores) != 0) {
  ...
  tryCatch({
    for (i in primeiraSensores$FLDCODIGO) {
      # Para cada sensor, busca pelas suas medicoes, onde o DTAREG est� entre o intervalo
      ...
      # quando o nome do sensor (coluna) j� existe, altera o modo da tabela para NAO criar 2 colunas para o mesmo sensor (ocorre quando h� + de 1 measurement)
  ...
  tryCatch({
    for (iC in 2:ncol(primeiraDados)) {
      ...
      # O nome do segundo sensor � o FLDNAME da segundaTorre, buscando pelo FLDNAME que possui o canal EPE igual ao primeiro sensor 
      ...
      # Se o canal EPE for inexistente (ocorre quando � uma bateria, anemometro vertical...), utiliza o prefixo do primeiro sensor na busca
      if(is.na(segundaFld)) {
      ...
      if(segundaFld %in% names(segundaDados)) {
        ...
        # Atribui a descricao EPE do canal, at� encontrar um dos caracteres especiais="," "[" "(" e ".", para o titulo do plot
        tituloPlot <- trimws(word(primeiraSensores$EPECANAL[primeiraSensores$FLDNAME == primeiraFld][1],1,sep="\\,|\\[|\\(|\\."))
        
        # Seta os parametros do plot e o salva dentro do diretorio da correla��o
        plot <- ggplot() +
        ...
        segundaWarnings <- append(segundaWarnings,paste("O sensor",primeiraFld,",da torre",nomePrimeiraTorre,"n�o encontrou o sensor correspondente",segundaFld,",na torre",nomeSegundaTorre,"\n"))
      }
    } #iC in 2:ncol(primeiraDados)
  }, error = function(err) {
    message(paste0("Falha na gera��o dos plots:\n",err))
    message("-\n")
  })
